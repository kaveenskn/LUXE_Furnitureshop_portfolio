...
/* ─── Section-level animation hook ───
   Finds all [data-reveal] children inside `scopeRef` and animates them
   as a staggered batch when the section enters the viewport.
   No per-element ScrollTrigger — one trigger per section.          */
function useSectionReveal(scopeRef, { start = 'top 82%', stagger = 0.13, duration = 0.9, y = 48 } = {}) {
  useGSAP(() => {
...
/* ─── Card components (hover only, no per-element scroll trigger) ─── */
...
function GridTile({ icon, title, description }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div
      data-reveal
...
export default function Philosophy() {
  ...
  // Section refs — each gets one ScrollTrigger
  ...
  /* Hero entrance — timeline, not scroll-triggered */
  useGSAP(() => {
  ...
  /* ── Section-level reveals (one trigger each) ── */
  useSectionReveal(believeRef,  { stagger: 0.14 });
  useSectionReveal(cardsRef,    { stagger: 0.16 });
  useSectionReveal(splitRef,    { stagger: 0.14 });
  useSectionReveal(commitRef,   { stagger: 0.14 });
  useSectionReveal(gridRef,     { stagger: 0.12 });
  useSectionReveal(quoteRef,    { stagger: 0.15 });
  useSectionReveal(ctaRef,      { stagger: 0.13 });
  ...
  return (
    <div ref={pageRef} style={{ background: '#0a0a0a', color: '#fff', fontFamily: SANS, overflowX: 'hidden' }}>
      ...
      {/* ══════ WHAT WE BELIEVE ══════ */}
      <section ref={believeRef} className="phil-section">
        <div className="phil-container">
          <span data-reveal className="phil-label">What We Believe</span>
          <div className="believe-grid">
            <div data-reveal style={{ position:'relative', aspectRatio:'4/5', overflow:'hidden' }}>
              <img
      ...
      {/* ══════ PHILOSOPHY CARDS ══════ */}
      <section ref={cardsRef} style={{ padding:'0 0 120px' }}>
        <div className="phil-container" style={{ marginBottom:'64px' }}>
          <span data-reveal className="phil-label">Core Values</span>
          <h2 data-reveal className="phil-h2">Our Guiding Principles</h2>
        </div>
        <div style={{ borderTop:'1px solid rgba(255,255,255,0.08)', borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display:'flex', gap:'1px', background:'rgba(255,255,255,0.08)', flexWrap:'wrap' }}>
            <PhilosophyCard icon="◈" title="Precision" description="Every millimeter matters. Accurate engineering ensures smooth performance and effortless installation across every product we make." />
            <PhilosophyCard icon="◎" title="Innovation" description="We constantly refine our products to improve usability, durability, and the overall furniture experience through thoughtful R&D." />
            <PhilosophyCard icon="◇" title="Quality Without Compromise" description="From material selection to final inspection, every fitting reflects our commitment to excellence and long-term performance." />
          </div>
        </div>
      </section>
      ...
      <section ref={splitRef}>
        <div className="split-screen">
          ...
          <div style={{ background:'#111', display:'flex', alignItems:'center', padding:'80px 72px' }}>
            <div>
              <span data-reveal className="phil-label">The Hidden Craft</span>
              <h2 data-reveal className="phil-h2" style={{ marginBottom:'28px' }}>Beauty Starts<br />Beneath the Surface</h2>
              <div data-reveal className="phil-divider" />
              <p data-reveal className="phil-body" style={{ marginBottom:'20px' }}>
              ...
              <p data-reveal className="phil-body">
      ...
      {/* ══════ OUR COMMITMENT ══════ */}
      <section ref={commitRef} style={{ position:'relative', padding:'140px 0', overflow:'hidden', textAlign:'center' }}>
        ...
        <div className="phil-container" style={{ position:'relative', zIndex:2 }}>
          <span data-reveal className="phil-label" style={{ textAlign:'center', display:'block' }}>Our Commitment</span>
          <p data-reveal style={{ fontFamily:SERIF, fontWeight:400, fontSize:'clamp(1.3rem,2.5vw,2rem)', lineHeight:1.65, color:'rgba(255,255,255,0.88)', maxWidth:'860px', margin:'0 auto', letterSpacing:'-0.01em' }}>
          ...
          <div data-reveal style={{ width:'64px', height:'1px', background:'rgba(255,255,255,0.3)', margin:'48px auto 0' }} />
        </div>
      </section>

      {/* ══════ PHILOSOPHY GRID 2x2 ══════ */}
      <section ref={gridRef} className="phil-section">
        <div className="phil-container">
          <span data-reveal className="phil-label">How We Work</span>
          <h2 data-reveal className="phil-h2" style={{ marginBottom:'64px', maxWidth:'480px' }}>Four Pillars of Excellence</h2>
        </div>
        <div className="phil-container" style={{ padding:0 }}>
          <div className="phil-grid-2x2">
            <GridTile icon="⬡" title="Precision Engineering" description="Our products are manufactured to tolerances invisible to the naked eye. Every spec, every groove, every fastener point is exactingly controlled." />
            <GridTile icon="◈" title="Long-Term Durability" description="We select materials that endure decades of daily use without compromise—tested to outlast trends and exceed industry standards." />
            <GridTile icon="○" title="Thoughtful Innovation" description="Innovation at our company means solving real problems—quieter closes, easier installation, longer service life. No change for change's sake." />
            <GridTile icon="◇" title="Customer-Centered Design" description="We listen to craftsmen, furniture makers, and homeowners alike. Every insight from the field feeds back into how we design and refine our products." />
          </div>
        </div>
      </section>

      {/* ══════ QUOTE ══════ */}
      <section ref={quoteRef} style={{ background:'#050505', borderTop:'1px solid rgba(255,255,255,0.06)', borderBottom:'1px solid rgba(255,255,255,0.06)', padding:'140px 0', textAlign:'center' }}>
        <div className="phil-container">
          <div data-reveal style={{ marginBottom:'32px', opacity:0.2 }}>
            <span style={{ fontFamily:SERIF, fontSize:'6rem', lineHeight:0.5, color:'#fff', display:'block' }}>"</span>
          </div>
          <blockquote data-reveal style={{ fontFamily:SERIF, fontWeight:400, fontSize:'clamp(1.25rem,2.8vw,2.1rem)', lineHeight:1.6, color:'rgba(255,255,255,0.9)', maxWidth:'820px', margin:'0 auto', letterSpacing:'-0.01em', fontStyle:'italic' }}>
          ...
          <div data-reveal style={{ marginTop:'48px', display:'flex', alignItems:'center', justifyContent:'center', gap:'16px' }}>
      ...
      {/* ══════ CTA ══════ */}
      <section ref={ctaRef} className="phil-section" style={{ textAlign:'center' }}>
        <div className="phil-container">
          <span data-reveal className="phil-label">Take the Next Step</span>
          <h2 data-reveal className="phil-h2" style={{ marginBottom:'20px' }}>Ready to build furniture<br />that lasts?</h2>
          <p data-reveal className="phil-body" style={{ maxWidth:'500px', margin:'0 auto 52px' }}>
            Discover our complete range of precision-engineered fittings or speak with our team about your next project.
          </p>
          <div data-reveal className="cta-btns">
